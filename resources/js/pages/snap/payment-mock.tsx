...
export default function PaymentMock({
    ...
    const handleSimulatorAction = async (
        action: "settle" | "expire" | "cancel" | "deny",
    ) => {
        ...
        try {
            const response = await fetch(`/simulator/action/${action}`, {
                ...
                body: JSON.stringify({
                    ...
                    payment_type:
                        selectedTab === "va" ? "bank_transfer" : selectedTab,
                }),
            });
            ...
            if (response.ok) {
                if (action === "settle") {
                    ...
                    toast.success(
                        "Simulasi Pembayaran Sukses! Webhook telah dikirim ke merchant.",
                    );
        ...
        <div className="min-h-screen bg-neutral-950 text-neutral-100 flex flex-col font-sans selection:bg-blue-500/30 selection:text-blue-300">
            ...
            <main className="flex-1 max-w-4xl w-full mx-auto p-4 sm:p-6 lg:p-8 flex flex-col justify-center">
                <div className="bg-neutral-900/90 border border-neutral-800/80 rounded-2xl shadow-2xl overflow-hidden backdrop-blur-xl">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-neutral-400 mt-1 flex items-center gap-2">
                                ...
                                <span className="font-mono text-neutral-500">
                                    UUID: {transaction.id.substring(0, 8)}...
                                </span>
                            </div>
                        </div>
                    ...
                    {status === "settlement" && (
                        <div className="p-8 text-center bg-linear-to-b from-emerald-950/30 to-neutral-900 border-b border-emerald-900/30 flex flex-col items-center">
                            ...
                            <p className="text-sm text-neutral-400 max-w-md mt-2">
                                Simulasi transaksi telah berstatus{" "}
                                <strong>Settlement</strong>. Webhook notifikasi
                                Midtrans SHA-512 telah berhasil dikirim ke
                                server merchant.
                            </p>
                    ...
                    {status === "expire" && (
                        <div className="p-8 text-center bg-neutral-950/50 border-b border-amber-900/30 flex flex-col items-center">
                            ...
                            <p className="text-xs text-neutral-400 max-w-md mt-1">
                                Batas waktu pembayaran telah habis. Status
                                transaksi adalah <code>expire</code> (HTTP 407).
                            </p>
                        </div>
                    )}
                    ...
                    {status === "cancel" && (
                        <div className="p-8 text-center bg-neutral-950/50 border-b border-rose-900/30 flex flex-col items-center">
                            ...
                            <p className="text-xs text-neutral-400 max-w-md mt-1">
                                Transaksi ini telah dibatalkan dengan status{" "}
                                <code>cancel</code> (HTTP 202).
                            </p>
                        </div>
                    )}
                    ...
                    <div className="border-b border-neutral-800/80 px-6 py-3 bg-neutral-950/30">
                        ...
                        >
                            <span>
                                Rincian Pembelian & Pelanggan (
                                {transaction.item_details?.length || 1} item)
                            </span>
                            <span className="flex items-center gap-1">
                                {showItems ? "Tutup" : "Lihat"}{" "}
                                {showItems ? (
                        ...
                        {showItems && (
                            <div className="pt-3 pb-2 text-xs text-neutral-300 space-y-2 border-t border-neutral-800/60 mt-2">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <div className="text-[11px] font-semibold uppercase text-neutral-500 mb-1">
                                            Item Pesanan
                                        </div>
                                        {transaction.item_details &&
                                        transaction.item_details.length > 0 ? (
                                            transaction.item_details.map(
                                                (item, idx) => (
                                                    <div
                                                        key={idx}
                                                        className="flex justify-between py-1 border-b border-neutral-800/40"
                                                    >
                                                        <span>
                                                            {item.name} x
                                                            {item.quantity}
                            ...
                            {selectedTab === "qris" && (
                                <div className="flex flex-col items-center text-center space-y-6">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <input
                                                ...
                                                value={
                                                    ...
                                                    "00020101021226590014ID.LINKAJA..."
                                                }
                                    ...
                                    <button
                                        ...
                                        disabled={actionLoading !== null}
                                        className="w-full max-w-md py-3 rounded-xl bg-linear-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-semibold text-sm shadow-xl shadow-emerald-950/60 transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-98"
                                    >
                                        {actionLoading === "settle" ? (
                                        ...
                                        {[
                                            {
                                                ...
                                                desc: "Scan QR atau Buka Aplikasi Gojek",
                                            },
                                            {
                                                id: "shopeepay",
                                                name: "ShopeePay",
                                                desc: "Buka Aplikasi Shopee",
                                            },
                                            {
                                                id: "ovo",
                                                name: "OVO SmartPay",
                                                desc: "Notifikasi Push OVO",
                                            },
                                            {
                                                id: "dana",
                                                name: "DANA Dompet Digital",
                                                desc: "Redirect ke Saldo DANA",
                                            },
                                        ].map((ew) => (
                                            <div
                                                key={ew.id}
                                ...
                                <div className="space-y-6">
                                    <div className="p-5 rounded-xl bg-neutral-950 border border-neutral-800/80 space-y-4 text-center">
                                        ...
                                        <div className="text-xs text-neutral-400 border-t border-neutral-800 pt-3">
                                            Tunjukkan kode pembayaran ini kepada
                                            kasir Indomaret / Alfamart terdekat.
                                        </div>
                                    </div>
                    ...
                    <div className="bg-neutral-950/80 border-t border-neutral-800/60 px-6 py-3.5 flex flex-col sm:flex-row items-center justify-between text-[11px] text-neutral-500 gap-2">
                        <div className="flex items-center gap-1.5">
                            ...
                            <span>
                                Mock Payment Gateway Simulator (Midtrans
                                Architecture)
                            </span>
                        </div>
                        <div className="font-mono">
                            SHA-512 Signed • Merchant Code:{" "}
